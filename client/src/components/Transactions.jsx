...
const TransactionCard = ({addressTo, addressFrom, timestamp, message, amount}) => {
    ...
    return (
        <div className={"white-glassmorphism w-full flex p-3 h-[230px] rounded-xl hover:shadow-2xl "}>
                ...
                <div className={"flex flex-col mb-2 justify-between w-full p-2 "}>
                    <div className={"space-y-1"}>
                        ...
                        {message && (
                            <>
                                <br/>
                                <p className={"text-white text-base"}>Message: <span className={"ml-1"}>{message}</span></p>
                            </>
                        )}
                    </div>
...
const Transactions = () => {
    ...
    const {currentAccount, addressTo, amount} = useTransactionContext();
            ...
            <div className={"max-w-7xl mx-auto w-full"}>
                <div className={"flex flex-col justify-center items-center 2xl:px-20 px-4 py-12"}>
                    ...
                    <div className={"grid grid-cols-1 w-full my-4 sm:grid-cols-2 md:grid-cols-3 gap-4 text-white"}>
                        {data.map((transaction,index) => <TransactionCard key={index} {...transaction} />)}
                    </div>
                </div>
            </div>
        </section>
